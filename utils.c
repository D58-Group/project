...
void print_hdr_tcp(uint8_t* buf) {
  ...
  int first = 1;
  if (flags & 0x01) {
    fprintf(stderr, "%sFIN", first ? "" : "|");
    first = 0;
  }
  if (flags & 0x02) {
    fprintf(stderr, "%sSYN", first ? "" : "|");
    first = 0;
  }
  if (flags & 0x04) {
    fprintf(stderr, "%sRST", first ? "" : "|");
    first = 0;
  }
  if (flags & 0x08) {
    fprintf(stderr, "%sPSH", first ? "" : "|");
    first = 0;
  }
  if (flags & 0x10) {
    fprintf(stderr, "%sACK", first ? "" : "|");
    first = 0;
  }
  if (flags & 0x20) {
    fprintf(stderr, "%sURG", first ? "" : "|");
    first = 0;
  }
